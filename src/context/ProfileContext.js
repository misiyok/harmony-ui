...
import {
  do_fetchAllSkills,
  do_persistProfile,
  do_persistEditedProfile,
  do_fetchPotentialMatches,
  do_fetchUserProfileInfo,
  do_like,
  do_dislike,
  do_uploadImage,
  do_uploadNewImage,
  ...
  switch (action.type) {
    ...
    case 'set_profile':
      return {
        ...
        profilePhotoUri: action.payload.profilePhotoUri,
      };
    case 'clean_state':
      return {
        userId: null,
...
const _uploadImage = (dispatch) => {
  return async (userId, uri) => {
    await do_uploadImage(userId, uri, (type, payload) => {
      dispatch({ type, payload });
    });
  };
};

const _changeImage = (dispatch) => {
  return async (userId, uri) => {
    await do_uploadNewImage(userId, uri, (type, payload) => {
      dispatch({ type, payload });
    });
  };
};
